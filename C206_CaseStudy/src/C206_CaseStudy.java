...
public class C206_CaseStudy {
	...
	public static void main(String[] args) {
		...
		public static void deleteAppointment(ArrayList<Appointment> appointmentList) {
			...
			for (int i = 0; i < appointmentList.size(); i++) {
				if (appointmentList.get(i).getCustomerID() == delete) {
		...
		public static void searchAppointment(ArrayList<Appointment> appointmentList) {
			...
			int search = Helper.readInt("Please enter Appointment ID to search: ");

			for (int i = 0; i < appointmentList.size(); i++) {
				if (appointmentList.get(i).getCustomerID() == search) {
					System.out.println("Customer name: " + appointmentList.get(i).getCustomerName());
					System.out.println("Customer ID: " + appointmentList.get(i).getCustomerID());
					System.out.println("Time of Appointment: " + appointmentList.get(i).getAppointmentDate());
				} else {
		...
		public static void updateAppointment(ArrayList<Appointment> appointmentList) {
			C206_CaseStudy.setHeader("UPDATE APPOINTMENT ONE DAY BEFORE THE APPOINTMENT DATE");
			String custName = Helper.readString("Enter name > ");
			int custID = Helper.readInt("Enter ID > ");
			String input = Helper.readString("Updated time of meeting (dd/MM/yy hh:mm) > ");
			DateTimeFormatter formatter1 = DateTimeFormatter.ofPattern("dd/MM/yyyy hh:mm");
			LocalDateTime new_date = LocalDateTime.parse(input, formatter1);
			boolean isUpdated = false;

			for (int i = 0; i < appointmentList.size(); i++) {
				if (isUpdated == false && custName.equalsIgnoreCase(appointmentList.get(i).getCustomerName())) {
					appointmentList.get(i).setCustomerName(custName);
					appointmentList.get(i).setCustomerID(custID);
					appointmentList.get(i).setAppointmentDate(new_date);
